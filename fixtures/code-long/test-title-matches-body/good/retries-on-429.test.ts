// The same shape and length as the bad retry test, and the late assertions are the ones the title
// promises: how many calls were made, and when the retry went out.
...
test("retries a request once when the server answers 429", async () => {
  ...
  assert.equal(calls.length, 3, "the 429 costs one extra call, and the second page costs one more");
  assert.equal(calls[1]!.at, 1000, "the retry waits the second the server asked for in retry-after");
  assert.equal(calls[1]!.url, calls[0]!.url, "the retry repeats the same request");
  assert.equal(everything.length, 12);
});
