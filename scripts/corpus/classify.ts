// Runs every HTML rule over the corpus and keeps every classification, reported or not. Precision needs a
// sample of what was reported; recall needs a sample of what was not, so both are saved.
//
//   varlock run -- node scripts/corpus/classify.ts [--corpus corpus]
//   node scripts/corpus/classify.ts --cache-only      re-read after a change to code or policy; asks nothing
//
// Writes <corpus>/classifications.json (gitignored: it quotes other people's pages) and prints, per rule, how
// many candidates were classified, how many were reported at each severity, where the probabilities fall,
...
const { classifications, findings, stats } = await run(files, {
  rules: htmlRules,
...
const saved = classifications.map((j, index) => {
...
writeFileSync(`${values.corpus}/classifications.json`, JSON.stringify(saved));
...
console.log(
  `${files.length} pages, ${saved.length} classifications, ${findings.length} reported, ${stats.unanswered} unclassified. ${stats.questions} questions, ${stats.cacheHits} cached, ${stats.requests} requests, ${stats.inputTokens.toLocaleString("en")} tokens, $${cost.toFixed(2)}, ${Math.round((Date.now() - started) / 1000)}s`,
);
