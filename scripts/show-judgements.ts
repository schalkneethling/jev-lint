// Prints every candidate a rule judged, reported or not, with its probability and the words Jev saw.
// A clean report says nothing about how close the calls were; this does.
// Usage: varlock run -- node scripts/show-judgements.ts <file|url> <rule-id>
...
const { judgements } = await run([{ path: target!, source }], {
...
for (const j of judgements.toSorted((a, b) => b.p - a.p)) {
