...
export default defineCodeRule({
  ...
  // Accuracy can only be judged for a comment that says what the code does. A comment that gives a
  ...
  questions: (ref) => ({
    ...
    describes_behaviour: noul(
      `${ref("comment")} is a comment in source code. Judge the comment by itself and ignore any code: does it describe an operation that code performs?`,
      {
